<%@ Page Title="Piczard Examples | Web - PictureTrimmer | Client side load, unload and save images" Language="VB" MasterPageFile="~/design/masters/DefaultMasterPage.master" AutoEventWireup="false" CodeFile="default.aspx.vb" Inherits="examples_example_A_307_default" %>
    ...
    <div class="pageContainer">
        ...
        <script type="text/javascript">
            ...
            function InlinePictureTrimmer1_LoadImage()
            {
                ...
                var oInlinePictureTrimmer = CodeCarvings.Wcs.Piczard.PictureTrimmer.getControl("<% =Me.InlinePictureTrimmer1.ClientID %>");
                
                var url = "<% =Me.ResolveUrl("LoadImageHelper.ashx") %>?timestamp=" + CodeCarvings.Wcs.Piczard.Shared.Helpers.TimeHelper.getTimeStamp();

				var dataToBeSent = {
					coreJSONString: oInlinePictureTrimmer.get_coreJSONString(),
					imageToLoad: $("#<% =Me.ddlInlineImageToLoad.ClientID %>").val()
				};
            ...
            function InlinePictureTrimmer1_SaveImage()
            {
                ...
                var url = "<% =Me.ResolveUrl("SaveImageHelper.ashx") %>?timestamp=" + CodeCarvings.Wcs.Piczard.Shared.Helpers.TimeHelper.getTimeStamp();
            ...
            {
                // Get the PictureTrimmer instance
                var oPopupPictureTrimmer = CodeCarvings.Wcs.Piczard.PictureTrimmer.getControl("<% =Me.PopupPictureTrimmer1.ClientID %>");
                
                var url = "<% =Me.ResolveUrl("LoadImageHelper.ashx") %>?timestamp=" + CodeCarvings.Wcs.Piczard.Shared.Helpers.TimeHelper.getTimeStamp();

				var dataToBeSent = {
					coreJSONString: oPopupPictureTrimmer.get_coreJSONString(),
					imageToLoad: $("#<% =Me.ddlPopupImageToLoad.ClientID %>").val()
				};
        ...
        <div class="InlinePanel1" style="padding:5px; margin-top:10px;">
            <span id="spanInlineCommands1" style="<% =If(Me.InlinePictureTrimmer1.ImageLoaded, "none", "inline") %>">
                Image to load: 
            ...
            <span id="spanInlineCommands2" style="<% =If(Me.InlinePictureTrimmer1.ImageLoaded, "inline", "none") %>">
